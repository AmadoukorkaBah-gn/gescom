...
<div class="container mx-auto px-4 py-8">
    <div class="max-w-lg mx-auto">
        <h1 class="text-2xl font-bold text-gray-800 mb-6">Créer un Nouvel Administrateur</h1>
        
        <form action="{{ route('super-admin.store-admin') }}" method="POST" class="bg-white shadow-md rounded-lg p-6">
            @csrf
            ...
            <div class="mb-4">
                ...
                <input type="password" name="password" 
                       class="form-input w-full border rounded px-3 py-2 @error('password') border-red-500 @enderror" required>
                @error('password')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
