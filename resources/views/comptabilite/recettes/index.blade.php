...
<div class="container mx-auto px-4 py-8">
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-2xl font-bold text-gray-800">Liste des Recettes</h1>
        <a href="{{ route('recettes.create') }}" class="bg-green-500 hover:bg-green-600 text-white px-4 py-2 rounded">
            Nouvelle Recette
        </a>
    </div>
        ...
        <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                ...
                <tr class="hover:bg-gray-50">
                    <td class="px-4 py-3 text-sm">{{ $recette->date_recette->format('d/m/Y à H:i') }}</td>
                    <td class="px-4 py-3 text-sm font-medium">{{ $recette->libelle }}</td>
                    <td class="px-4 py-3 text-sm">{{ $recette->caisse->nom ?? '-' }}</td>
                    ...
                    <td class="px-4 py-3 text-sm flex space-x-1">
                        ...
                        <form action="{{ route('recettes.destroy', $recette->id) }}" method="POST" class="inline">
                            @csrf
                            @method('DELETE')
                            <button type="submit" 
                ...
                @empty
                <tr>
                    <td colspan="5" class="px-4 py-3 text-center text-gray-500">Aucune recette trouvée</td>
                </tr>
                @endforelse
            </tbody>
        </table>
